...
const Shop = observer(() => {
  ...
  useEffect(() => {
    ...
    fetchDevices(device.selectedType.id, device.selectedBrand.id, device.limit, device.page).then(data => {
      device.setDevices(data.rows.slice())
      device.setTotalCount(data.count)
  ...
  return (
    <Container>
      <Row className='mt-5'>
       ...
       <CreateDevice purpose='edit' show={modals.deviceModalVisible} onHide={() => modals.setDeviceModalVisible(false)}></CreateDevice> 
    </Container>
  )
})
