      ...
      <div>
        <h4>Типы</h4>
        {
          device.types.map(type => (
            <div className=' mb-2 d-flex aling-items-center justify-content-between'
              key={type.id}
            >
              {type.name}
        ...
        <Button
          ...
          className='pb-2 mb-2'>Добавить тип</Button>
      </div>
      <div>
        <h4 >Бренды</h4>
        {
          device.brands.map(brand => (
            <div className=' mb-2 d-flex aling-items-center justify-content-between'
              key={brand.id}
            >{brand.name}
              <Button onClick={() => handleDeleteBrand(brand.id)}>удалить</Button></div>
          ))
        }
        <Button
          onClick={() => modals.setBrandModalVisible(true)}
          variant={'outline-dark'}
          className='pb-2 mb-2'>Добавить бренд</Button>
      </div>


      <Button onClick={() => modals.setDeviceModalVisible(true)} variant={'outline-dark'} className='pb-2 mb-2'>Добавить устройство</Button>

      <CreateType
        updateTypes={updateTypes}
        show={modals.typeModalVisible}
        onHide={() => modals.setTypeModalVisible(false)}></CreateType>
      <CreateDevice
        purpose='create'
        show={modals.deviceModalVisible}
        onHide={() => modals.setDeviceModalVisible(false)}></CreateDevice>
      <CreateBrand
        updateBrands={updateBrands}
        show={modals.branModaldVisible}
        onHide={() => modals.setBrandModalVisible(false)}></CreateBrand>
    </Container>
  )
})
